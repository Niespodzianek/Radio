...
# Aktualna wersja projektu: 0.0.1.
...
def nowe_polaczenie(dotychczasowe_dane):
    ...
    dotychczasowe_dane.append([czas_utc, kod_Q, ZNAK, znak_korespondenta, radio, karta, czas_lokalny])
...
def nowy_nasłuch(dotychczasowe_dane):
    ...
    while wpisywanie:
        ...
        if znak_korespondent_pierwszy != znak_korespondent_drugi:
            ...
            kod_Q = wybor(KODY_Q)
            karta = wybor(KARTA_SQL)
            dotychczasowe_dane.append([czas_utc, kod_Q, znak_korespondent_pierwszy, znak_korespondent_drugi, radio, karta, czas_lokalny])
            nowe_dane_polaczen = dotychczasowe_dane
            return nowe_dane_polaczen
        else:
...
def menu(dane_polaczen):
    ...
    if wybor_menu == "1":
        ...
        if len(dane_polaczen) == 0:
            print("Dotychczas jeszcze nie było żadnych !!!!!! Logbook jest pusty.")
        else:
